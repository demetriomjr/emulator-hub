...
2. Configure `HOST` and `PORT` in `apps/backend/.env`, then start the backend from `apps/backend` with `npm start` (the provided local configuration is `http://127.0.0.1:3001`).
