...
### Local recovery capture interval and synchronous work (2026-09-24)

The live local-recovery capture starts immediately when the player becomes ready and then repeats every 10 seconds (previously 2.5 seconds). This reduces routine state serialization frequency while accepting up to roughly 10 seconds of local-only progress loss after an unexpected exit. Existing startup-choice delays, close cleanup, cloud recovery, user-state, and canonical `.sav` behavior are unchanged. Earlier 2.5-second descriptions in this dated audit are historical.

The pinned EmulatorJS 4.2.3 `GameManager.getState()` calls the core's synchronous `saveStateInfo()` export and copies bytes from the WASM heap before returning. `captureLocalRecovery()` being `async` does not make that call asynchronous. The store's IndexedDB transaction is asynchronous, but state normalization and multiple byte-array/structured clones before the transaction are synchronous. The same `getState()` path is used by remote state capture. There is no compatible asynchronous `getState()` API in this pinned runtime. Deferring the call with a Promise or timer would only move the blocking work to a later turn; a true off-thread capture would require moving the core/serialization into a worker or a supported threaded runtime and separate validation. No such runtime change is part of this scoped update.

| Runtime path in the current frontend | Synchronous segment | Existing asynchronous segment / decision |
| --- | --- | --- |
| Local recovery (now immediate + 10 s) and 15-second cloud recovery | `getState()` serializes in the core and copies the WASM bytes; local normalization and structured clones also copy data. | IndexedDB transaction and cloud HTTP upload are asynchronous. No core-side async capture API is available in 4.2.3. |
| Canonical `.sav` poll and final flush | `saveSaveFiles()` and `getSaveFile()` execute core/virtual-FS work synchronously. The minimum poll interval is now 3 seconds (previously 1 second). | Hashing and HTTP upload are asynchronous after bytes have been read. The final close flush remains in place. |
| Startup restore and manual Load state | `loadState()`, `restart()`, and the virtual-FS write/load calls execute synchronously. Wrapping `restart()` in `Promise.resolve()` does not move the call off-thread. | Backend/IndexedDB reads are asynchronous before the core call; the established `.sav` restore order is preserved. |
| Input, pause, Fast Forward | `simulateInput()`, `toggleMainLoop()`, and Fast Forward ratio/toggle call core exports synchronously. | They are brief control calls; changing their timing would affect input and lifecycle ordering and requires separate evidence. |
