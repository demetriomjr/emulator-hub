...
- The surface uses a zero-gap, zero-outer-padding grid. Instances fill in order: one occupies the first cell; two fill row one; three and four use two columns and two rows; five uses two columns and three rows; six uses three columns and two rows so all six screens are arranged horizontally across each row. Each iframe remains its own `player.html` document and preserves its profile-scoped EmulatorJS game ID and save isolation.
- Size the surface from the number of occupied rows and columns so every occupied cell retains the existing 3:2 aspect ratio. At five instances, the surface remains square; at six, the surface has a 9:4 aspect ratio. The complete surface remains the fullscreen target and stays centered/proportional within the available viewport.
...
3. Placement is ordered and gapless: five instances use two columns and three rows; six instances use three columns and two rows. Every iframe retains a 3:2 ratio, and the six-instance surface is horizontal.
