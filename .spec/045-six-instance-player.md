...
- The surface uses a zero-gap, zero-outer-padding, two-column grid. Instances fill in order: one occupies the first cell; two fill row one; three and four fill two rows; five and six fill three rows. Each iframe remains its own `player.html` document and preserves its profile-scoped EmulatorJS game ID and save isolation.
- Size the surface from the number of occupied rows and columns so every occupied cell retains the existing 3:2 aspect ratio. At five instances, the final row has one occupied cell; at six, it is full. The complete surface remains the fullscreen target and stays centered/proportional within the available viewport.
...
3. Two-column placement is ordered and gapless for counts one through six, including the partially occupied third row at five and full third row at six. Every iframe retains a 3:2 ratio.
