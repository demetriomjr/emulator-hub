...
## Application deployment — 2026-09-26

After explicit build authorization, the VPS checkout at `dd237c6` was deployed with `docker compose --env-file deploy/.env -f docker-compose.yml up -d --build`. Before the build, a read-only archive of the persistent `emulator-hub-data` volume was verified at `/home/deploy/emulator-hub/backups/pre-nine-player-dd237c6.tgz` (SHA-256 `0747ebce52f6c019652939e96ec02ac96004ef4faaeeebd37811cc92739e7136`). Both images built and the backend became healthy before the frontend started. The served frontend assets contain the configured ninth port and the 3 × 3 player CSS.

From outside the VPS, HTTPS `/player.html` returned 200 with `Document-Isolation-Policy: isolate-and-require-corp` on every player port 8444–8452. The Hub `/` and `/api/games` returned 200; `/` remained unisolated. On each new port 8450–8452, `/api/games` returned 200 and a deliberately nonexistent ROM route returned the backend's 404. These checks establish routing and document policy; a browser run of nine concurrent games, renderer placement, lease/save persistence, local recovery and snapshot restoration remains unverified.
