...
The follow-up operations task extended the existing Caddy configuration on the Kali VPS. `/opt/caddy/compose.yaml` now publishes TCP 8450–8452 in addition to 8444–8449; `/opt/caddy/sites/pokehub.caddy` sends the three new HTTPS listeners to the existing `frontend:8080` service. The Git-ignored `/home/deploy/emulator-hub/deploy/.env` now sets `PLAYER_ORIGIN_PORTS=8444,8445,8446,8447,8448,8449,8450,8451,8452`. The configuration backup is `/home/deploy/nine-player-backup-20260926T185923Z` and includes the former Caddy Compose file, Caddy site file and `.env` with restricted permissions.
