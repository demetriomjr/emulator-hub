...
- The profile record already contains `id`, `name`, `createdAt`, and `oddsResetCount`. `PATCH /api/games/:gameId/profiles/:profileId` already accepts `{ "name": "..." }`, validates a printable 1–32 character name, rejects duplicates, and returns the updated profile.
...
6. Keep the existing name validation and duplicate-name behavior. No additional profile fields, views, or controls are included.
