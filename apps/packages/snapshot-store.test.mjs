...
async function bundle(state, save) {
  return {
    metadata: { core: 'gba', romSha256: 'a'.repeat(64), runtimeId: 'emulatorjs-4.2.3' },
    state: new Uint8Array(state),
    save: new Uint8Array(save),
  }
}

test('keeps only the latest atomically replaced snapshot bundle', async () => {
  ...
  const second = await store.put('profile-may', 'pokemon-emerald', await bundle([3], [4]), first.revision, { fenceGeneration: 1 })
  const current = await store.get('profile-may', 'pokemon-emerald')

  assert.equal(first.revision, 1)
  assert.equal(second.revision, 2)
  assert.deepEqual([...current.state], [3])
  assert.deepEqual([...current.save], [4])
...
test('rejects stale revisions and fenced snapshot writers without replacing the slot', async () => {
  const dataPath = await mkdtemp(join(tmpdir(), 'emulator-hub-snapshot-store-'))
  const store = createSnapshotStore({ dataPath })
  await store.put('profile-may', 'pokemon-emerald', await bundle([1], [2]), null, { fenceGeneration: 1 })

  await assert.rejects(
    store.put('profile-may', 'pokemon-emerald', await bundle([3], [4]), null, { fenceGeneration: 1 }),
    error => error.code === 'SNAPSHOT_REVISION_CONFLICT',
  )
  await store.advanceFence('profile-may', 'pokemon-emerald', 2)
  await assert.rejects(
    store.put('profile-may', 'pokemon-emerald', await bundle([3], [4]), 1, { fenceGeneration: 1 }),
...
test('deletes only the matching profile and game snapshot slot', async () => {
  ...
  const store = createSnapshotStore({ dataPath })
  await store.put('profile-may', 'pokemon-emerald', await bundle([1], [2]), null, { fenceGeneration: 1 })
  await store.put('profile-leaf', 'pokemon-emerald', await bundle([3], [4]), null, { fenceGeneration: 1 })
