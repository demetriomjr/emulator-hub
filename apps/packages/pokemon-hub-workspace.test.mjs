...
test('a pending save selection exclusively activates the save source mode', async () => {
  const workspace = await import('./pokemon-hub-workspace.mjs')

  assert.equal(typeof workspace.activePaneSourceKind, 'function')
  assert.equal(workspace.activePaneSourceKind({ kind: 'hub', hubProfileId: 'general' }, { kind: 'game' }), 'game')
