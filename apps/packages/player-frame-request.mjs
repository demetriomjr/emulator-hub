export function requestPlayerFrame({ frame, browser, sessionId, type, replyType, details = {}, timeoutMs = 30000 }) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const receive = event => {
      if (event.origin !== browser.location.origin || event.source !== target || event.data?.type !== replyType || event.data.requestId !== requestId || event.data.sessionId !== sessionId) return
    ...
    try { target.postMessage({ type, ...details, requestId, sessionId }, browser.location.origin) }
