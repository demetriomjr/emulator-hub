export function parsePlayerOriginPorts(location, value) {
  ...
  if (ports.length !== 9 || ports.some(port => !Number.isInteger(port) || port < 1 || port > 65535 || port === hubPort) || new Set(ports).size !== 9) {
    throw new Error('Player origin ports must be nine distinct valid ports, excluding the Hub port')
  }
  return ports
}

export function availablePlayerOriginSlot(sessions, maximum = 9) {
