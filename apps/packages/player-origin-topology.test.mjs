...
test('configured player ports select six origins independent of page query or debug mode', () => {
  const ports = '8444,8445,8446,8447,8448,8449'
  assert.deepEqual(parsePlayerOriginPorts(new URL('https://hub.example.com/'), ports), [8444, 8445, 8446, 8447, 8448, 8449])
  assert.deepEqual(parsePlayerOriginPorts(new URL('https://hub.example.com/?anything=1'), ports), [8444, 8445, 8446, 8447, 8448, 8449])
  assert.deepEqual(parsePlayerOriginPorts(new URL('https://hub.example.com:8443/'), ports), [8444, 8445, 8446, 8447, 8448, 8449])
  ...
  assert.throws(() => parsePlayerOriginPorts(new URL('https://hub.example.com/'), '443,8445,8446,8447,8448,8449'))
  assert.throws(() => parsePlayerOriginPorts(new URL('https://hub.example.com/'), '8444,8444,8446,8447,8448,8449'))
...
test('players get stable distinct origins while free slots can be reused', () => {
  ...
  const ports = parsePlayerOriginPorts(hub, '5175,5176,5177,5178,5179,5180')
  ...
  assert.equal(availablePlayerOriginSlot(Array.from({ length: 6 }, (_, playerOriginSlot) => ({ playerOriginSlot }))), null)
})
