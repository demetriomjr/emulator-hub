...
export function normalizePartial(value) {
  ...
  if (keys.some(key => !['fastForwardSpeed', 'fastForwardEnabled', 'muted', 'triggerActions', 'initializeIfAbsent'].includes(key))) throw preferenceError('USER_PREFERENCES_INVALID')
  ...
  if ('muted' in value && typeof value.muted !== 'boolean') throw preferenceError('USER_PREFERENCES_INVALID')
...
function normalize(value) {
  if (!value || value.version !== 1 || !speeds.includes(value.fastForwardSpeed) || ('fastForwardEnabled' in value && typeof value.fastForwardEnabled !== 'boolean') || ('muted' in value && typeof value.muted !== 'boolean') || !value.triggerActions || !actions.includes(value.triggerActions.l2) || !actions.includes(value.triggerActions.r2)) throw preferenceError('USER_PREFERENCES_INVALID')
  return { version: 1, fastForwardSpeed: value.fastForwardSpeed, fastForwardEnabled: value.fastForwardEnabled ?? false, muted: value.muted ?? false, triggerActions: { l2: value.triggerActions.l2, r2: value.triggerActions.r2 } }
}

function copy(value) { return { version: 1, fastForwardSpeed: value.fastForwardSpeed, fastForwardEnabled: value.fastForwardEnabled, muted: value.muted, triggerActions: { ...value.triggerActions } } }
...
if raw == false then
  local initial = { version = 1, fastForwardSpeed = input.fastForwardSpeed or 1.5, fastForwardEnabled = input.fastForwardEnabled or false, muted = input.muted or false, triggerActions = { l2 = 'none', r2 = 'none' } }
  ...
  if input.initializeIfAbsent or input.muted ~= nil then redis.call('SET', KEYS[1], cjson.encode(initial)); return cjson.encode({ preferences = initial, initialized = true }) end
...
if current.muted == nil then current.muted = false end
...
if input.muted ~= nil then current.muted = input.muted end
...
return cjson.encode({ preferences = current, initialized = true })`,
  async memory({ keys, arguments: [encoded], get, set }) {
    ...
    if (!raw) {
      ...
      if (input.muted !== undefined) initial.muted = input.muted
      if (input.triggerActions) Object.assign(initial.triggerActions, input.triggerActions)
      if (input.initializeIfAbsent || input.muted !== undefined) { await set(keys[0], JSON.stringify(initial)); return JSON.stringify({ preferences: initial, initialized: true }) }
    ...
    if (input.muted !== undefined) current.muted = input.muted
