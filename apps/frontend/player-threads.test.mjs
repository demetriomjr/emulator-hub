...
test('player acknowledges lock, close and local recovery clear to its own session', async () => {
  ...
  assert.equal(locked, true)
  send({ type: 'emulator-hub:interaction-lock', requestId: 'unlock-3', sessionId: 's1', revision: 3, locked: false })
  assert.equal(locked, false)
  send({ type: 'emulator-hub:interaction-lock', requestId: 'late-lock-2', sessionId: 's1', revision: 2, locked: true })
  assert.equal(locked, false)
